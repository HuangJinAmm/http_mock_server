            ...
            NodeType::Node => {
                ...
                ui.horizontal(|ui| {
                    ...
                    if self.active {
                        if ui
                            .button("☑")
                            .on_hover_text("从服务器删除此条模拟规则，立即生效")
                            .clicked()
                        {
                            self.active = !self.active;
                            return Action::SyncToServer((self.id, self.active));
                        }
                    } else {
                        if ui
                            .button("☐")
                            .on_hover_text("更新内容发往服务器，立即生效")
                            .clicked()
                        {
                            self.active = !self.active;
                            return Action::SyncToServer((self.id, self.active));
        ...
        let drag_resp = self
            ...
            .ui(ui, self.sub_items.iter_mut(), |item, ui, handler| {
                ...
                match item.ui_impl(ui, selected_str, flilter, open, Some(handler)) {
                    ...
                    Action::SyncToServer(x) => {
                        sub_resp = Action::SyncToServer(x);
                    }
                    _ => {}
                }
                // }
