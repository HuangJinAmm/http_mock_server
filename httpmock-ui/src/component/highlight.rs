...
pub fn code_editor_ui_notool(ui: &mut egui::Ui, code: &mut String, language: &str) {
    ...
    ui.with_layout(egui::Layout::left_to_right(),|ui|{
        egui::ScrollArea::horizontal()
            ...
            .id_source("requset_ui_scroller_no_tool")
            .show(ui, |ui| {
                let _editor_rsp = ui.add_sized(
                    ui.available_size(),
...
pub fn highlight_layouter<'a>(
    ...
    move |ui: &egui::Ui, string: &str, _wrap_width: f32| {
        let layout_job = highlight(ui.ctx(), &theme, string, language);
        // layout_job.wrap.max_width = wrap_width; // no wrapping
        ui.fonts().layout_job(layout_job)
    }
