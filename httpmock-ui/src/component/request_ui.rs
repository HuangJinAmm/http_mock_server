...
pub struct NetTestUi {
    ...
    load_test: LoadTest,
    #[serde(skip)]
    sender: Option<Sender<(i64, ResponseUi)>>,
    #[serde(skip)]
    reciver: Option<Receiver<(i64, ResponseUi)>>,
}
...
// #[cfg(not(target_arch = "wasm32"))]
// impl From<Response> for ResponseUi {
...
impl NetTestUi {
    pub fn ui(&mut self, ui: &mut Ui) {
            ...
            if send.clicked() {
                ...
                match self.test_type {
                    ...
                    TestType::Load => {
                        ...
                        self.load_test.result_list = Vec::new();
                        self.load_test.result = LoadTestResult::default();
                        let requset = self.req.clone();
                        let sender_clone = self.sender.clone();
                        if let Some(sender) = sender_clone {
                            start_load_test(sender, times, reqs, requset);
                        }
                    }
                }
            }
        ...
        ui.horizontal(|ui| {
            ...
            ui.selectable_value(&mut self.test_type, TestType::Load, "负载测试");
        });
...
async fn send_load_test_request(sender: Sender<(i64, ResponseUi)>, client: Client, req: Request) {
    let start = Local::now().timestamp_millis();
...
fn start_load_test(sender: Sender<(i64, ResponseUi)>, times: u16, reqs: u32, req: RequestUi) {
    thread::Builder::new()
        .name("send_req_thread".to_string())
        .spawn(move || {
            let capacity: usize = (times as usize) * (reqs as usize);
            let mut sender_requset: Vec<Request> = Vec::with_capacity(capacity);
            ...
            let _respui = RT.block_on(async move {
                ...
                for _ in 0..times {
                    ...
                    for _ in 0..reqs {
                        ...
                        send_load_test_request(sender.clone(), client.clone(), req).await;
                    }
