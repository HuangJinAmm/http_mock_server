...
use crate::{component::mock_path_ui::MockPathUi, PORT};
...
pub fn get_mock(id:u64,versoin:u32) -> Option<MockPathUi> {
    if let Ok(db) = sled::open(get_db_name(id)) {
        if let Some(res_ivec) = db.get(key_to_ivec(versoin)).unwrap() {
            let res_obj :MockPathUi = convert_to_obj(res_ivec);
            return Some(res_obj);
        }
    }
    None
...
pub fn add_new_version_mockinfo(id:u64,mock_info:&MockPathUi) {
    if let Ok(db) = sled::open(get_db_name(id)) {
        let mut last_id=0;
...
fn convert_ivec_to_mock(v:IVec) -> MockPathUi {
    let v = v.to_vec();
    let obj = serde_json::from_slice::<MockPathUi>(v.as_slice()).unwrap();
    obj
}
...
mod tests {
    ...
    use crate::component::mock_path_ui::MockPathUi;
    ...
    fn test_add_mockinfo() {
        let mut v = MockPathUi::default();
        for i in 0..100 {
            v.remark = i.to_string(); 
    ...
    fn test_get() {
        ...
        if let Ok(db) = sled::open("100") {
            ...
            let v = v.to_vec();
            let m:MockPathUi= serde_json::from_slice(v.as_slice()).unwrap();
            dbg!(m);
        }
    }
